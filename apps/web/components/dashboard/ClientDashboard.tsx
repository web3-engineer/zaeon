...
type Invoice = {
    ...
    enelNumber: string;          // número/identificador ENEL
    inj_kWh: number;
    cons_kWh: number;
    saldo_kWh: number;           // saldo de créditos
    amount_R$: number;           // valor da fatura (positivo = pagar / negativo = crédito)
    status: "PAGA" | "EM ABERTO" | "CRÉDITO";
};

const MOCK_DAILY: Daily[] = [
    // últimos ~30 dias (amostra simples)
...
const MOCK_INVOICES: Invoice[] = [
    { id: "inv_2025_06", ref: "2025-06", enelNumber: "ENEL-938271", inj_kWh: 6700, cons_kWh: 7200, saldo_kWh: 500, amount_R$: 212.45, status: "PAGA" },
    { id: "inv_2025_07", ref: "2025-07", enelNumber: "ENEL-948622", inj_kWh: 7200, cons_kWh: 7000, saldo_kWh: 700, amount_R$: -85.10, status: "CRÉDITO" },
    { id: "inv_2025_08", ref: "2025-08", enelNumber: "ENEL-955103", inj_kWh: 7050, cons_kWh: 7300, saldo_kWh: 450, amount_R$: 135.90, status: "EM ABERTO" },
];
...
export default function ClientDashboard({ company }: { company: string }) {
    ...
    const { injTotal, consTotal, valueTotal, injAvgDay } = useMemo(() => {
        const slice = MOCK_DAILY.slice(-days);
    ...
    const last = MOCK_DAILY[MOCK_DAILY.length - 1];
    ...
    return (
        <main className="min-h-screen w-full px-6 md:px-10 py-8 text-white">
            ...
            <section className="grid gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                <KpiCard
                    title="Energia injetada (30d)"
                    value={`${injTotal.toFixed(0)} kWh`}
                    hint="Soma da inj ativa reportada"
                />
                <KpiCard
                    title="Consumo (30d)"
